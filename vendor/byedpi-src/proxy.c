...
/* --- SplitCord-Turkey patch: DNS-over-HTTPS via local forwarder ---
 * ciadpi normalde getaddrinfo() ile sistemin DNS sunucusunu (ISP'nin verdiği,
 * genelde 192.168.1.1 gibi bir yönlendirici) kullanır. ISP'ler DNS yanıtlarını
 * manipüle edebildiği için, önce SplitCordService'in çalıştırdığı, gerçek DoH
 * isteğini Google'a (https://dns.google/dns-query) yapan yerel bir yönlendiriciye
 * (127.0.0.1:53535) düz UDP DNS sorgusu gönderiyoruz (TLS'i burada değil, C#
 * tarafında hallediyoruz). Yönlendirici çalışmıyorsa (ör. eski bir build veya
 * servis kapalıysa) orijinal getaddrinfo() davranışına düşülür, hiçbir şey
 * kırılmaz. */

#define SC_DOH_FORWARDER_PORT 53535
/* Yönlendirici en fazla 2 sağlayıcıyı (Google, sonra Quad9) sırayla dener,
 * her biri için ~2sn payı var (bkz. DohForwarder.cs PerProviderTimeout) — burada
 * yeterince geniş bir pencere bırakıyoruz ki ikinci sağlayıcı denemesi bitmeden
 * ciadpi vazgeçip sistem çözümleyicisine düşmesin. */
// Yönlendirici (DohForwarder.cs) artık sırayla 5 sağlayıcı deniyor (her biri ~1.5 sn
// zaman aşımı) — en kötü senaryoda ~7.5 sn sürebilir. Bu değer o süreden kısa olursa
// ciadpi, yönlendirici hâlâ deneme yaparken erkenden vazgeçip getaddrinfo()'ya
// düşüyor ve sonraki sağlayıcılara hiç şans tanınmamış oluyordu.
#define SC_DOH_TIMEOUT_MS 10000

static int sc_doh_build_query(const char *host, int host_len, int qtype, uint8_t *out, size_t cap)
{
    size_t qi = 0;
    if (cap < 12) return -1;

    out[qi++] = 0x13; out[qi++] = 0x37; /* ID */
    out[qi++] = 0x01; out[qi++] = 0x00; /* flags: RD=1 */
    out[qi++] = 0x00; out[qi++] = 0x01; /* QDCOUNT=1 */
    out[qi++] = 0x00; out[qi++] = 0x00; /* ANCOUNT */
    out[qi++] = 0x00; out[qi++] = 0x00; /* NSCOUNT */
    out[qi++] = 0x00; out[qi++] = 0x00; /* ARCOUNT */

    int label_start = 0;
    for (int i = 0; i <= host_len; i++) {
        if (i == host_len || host[i] == '.') {
            int label_len = i - label_start;
            if (label_len <= 0 || label_len > 63 || qi + 1 + (size_t)label_len + 5 >= cap) return -1;
            out[qi++] = (uint8_t)label_len;
            memcpy(out + qi, host + label_start, label_len);
            qi += label_len;
            label_start = i + 1;
        }
    }
    out[qi++] = 0x00; /* root label */
    out[qi++] = (uint8_t)((qtype >> 8) & 0xff);
    out[qi++] = (uint8_t)(qtype & 0xff);
    out[qi++] = 0x00; out[qi++] = 0x01; /* QCLASS=IN */

    return (int)qi;
}

static int sc_doh_parse_answer(const uint8_t *msg, int len, int qtype, union sockaddr_u *addr)
{
    if (len < 12) return -1;
    ...
    int pos = 12;
    /* tek soruyu (QNAME + QTYPE(2) + QCLASS(2)) atla */
    while (pos < len && msg[pos] != 0) {
        if ((msg[pos] & 0xC0) == 0xC0) { pos += 2; goto sc_qdone; }
        pos += msg[pos] + 1;
    }
    pos += 1;
sc_qdone:
    ...
    for (int a = 0; a < ancount && pos < len; a++) {
        if ((msg[pos] & 0xC0) == 0xC0) {
            ...
            while (pos < len && msg[pos] != 0) pos += msg[pos] + 1;
            pos += 1;
        }
        ...
        pos += 10;
        if (pos + rdlen > len || rdlen < 0) return -1;

        if (rtype == qtype) {
            if (qtype == 1 && rdlen == 4) {
                memset(addr, 0, sizeof(*addr));
                addr->in.sin_family = AF_INET;
                memcpy(&addr->in.sin_addr, msg + pos, 4);
                return 0;
            }
            if (qtype == 28 && rdlen == 16) {
                memset(addr, 0, sizeof(*addr));
                addr->in6.sin6_family = AF_INET6;
                memcpy(&addr->in6.sin6_addr, msg + pos, 16);
                return 0;
            }
        }
        pos += rdlen;
...
/* Tek bir soru tipini (A ya da AAAA) yerel DoH yönlendiricisine sorar. */
static int sc_doh_query_once(const char *host, int host_len, int qtype, union sockaddr_u *addr)
{
    uint8_t query[320];
    int qlen = sc_doh_build_query(host, host_len, qtype, query, sizeof(query));
    if (qlen < 0) return -1;

    int fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (fd < 0) {
        LOG(LOG_S, "sc_doh: socket() failed\n");
        return -1;
    }

#ifdef _WIN32
    DWORD tv = SC_DOH_TIMEOUT_MS;
    setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv, sizeof(tv));
#else
    struct timeval tv = { .tv_sec = SC_DOH_TIMEOUT_MS / 1000, .tv_usec = (SC_DOH_TIMEOUT_MS % 1000) * 1000 };
    setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
#endif

    struct sockaddr_in dst = {0};
    dst.sin_family = AF_INET;
    dst.sin_port = htons(SC_DOH_FORWARDER_PORT);
    dst.sin_addr.s_addr = htonl(INADDR_LOOPBACK);

    if (sendto(fd, (const char *)query, qlen, 0, (struct sockaddr *)&dst, sizeof(dst)) < 0) {
        LOG(LOG_S, "sc_doh: sendto() failed, errno/wsa=%d\n", get_e());
        close(fd);
        return -1;
    }

    uint8_t answer[1024];
    int n = recvfrom(fd, (char *)answer, sizeof(answer), 0, 0, 0);
    close(fd);
    if (n <= 0) {
        LOG(LOG_S, "sc_doh: recvfrom() failed/timed out (qtype=%d), errno/wsa=%d\n", qtype, get_e());
        return -1;
    }

    int pr = sc_doh_parse_answer(answer, n, qtype, addr);
    LOG(LOG_S, "sc_doh: qtype=%d answer=%d bytes parse_result=%d\n", qtype, n, pr);
    return pr;
}

/* discord.com gibi birçok gerçek dünya alan adının AAAA kaydı yok (NODATA
 * yanıtı: NOERROR ama ANCOUNT=0) — ISP'nin yönlendiricisi bunun yerine
 * DNS64/NAT64 sentezlemesiyle sahte bir IPv6 uydurabiliyor, ama temiz bir DoH
 * sağlayıcısı (Google/Quad9) bunu yapmaz. Bu yüzden AAAA boş dönerse A
 * kaydına düşüyoruz — aksi halde ipv6 açıkken (varsayılan) AAAA'sı olmayan
 * her alan adı için DoH "başarısız" görünür ve gereksiz yere sistem
 * çözümleyicisine (ve olası DNS64 garipliklerine) düşülür. */
static int sc_doh_resolve(const char *host, int host_len, union sockaddr_u *addr)
{
    if (params.ipv6 && sc_doh_query_once(host, host_len, 28, addr) == 0) {
        return 0;
    }
    return sc_doh_query_once(host, host_len, 1, addr);
}
/* --- SplitCord-Turkey patch sonu --- */
        ...
        union sockaddr_u *addr, int type)
{
    ...
    if (!sc_doh_resolve(host, len, addr)) {
        LOG(LOG_S, "resolve (doh): %s ok\n", host);
        return 0;
    }
    LOG(LOG_S, "doh forwarder unavailable for %s, falling back to system resolver\n", host);
