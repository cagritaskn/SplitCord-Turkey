...
// KULLANICI RAPORLARI (hepsi AYNI kök nedene işaret ediyor): Tepsiye Küçült/Geri Al
// pencereyi küçültüp "anında" geri getiriyordu; Kamerayı Aç/Kapat kamerayı kapatıp
// anında tekrar açıyordu; Ekran Paylaşımını Aç/Kapat yayını durdurduktan hemen sonra
// seçim penceresini tekrar açıyordu VE yayın YOKKEN açılan seçim penceresi "İptal"e ya
// da "Paylaşımı Başlat"a basılsa bile hemen geri geliyordu. Hepsi TEK bir fiziksel tuş
// basışının altta İKİ KEZ tetiklenmesiyle birebir uyumlu: DOM'u yeniden sorgulayan
// TOGGLE_*_SCRIPT'ler (bkz. voiceState.js) ilk çağrıda doğru butonu bulup tıklıyor
// (kamera kapanıyor / yayın duruyor), ama DOM hemen SONRAKİ durumu yansıttığı için ikinci
// (yarış halindeki) çağrı KARŞIT butonu bulup tıklıyor (kamera tekrar açılıyor / seçim
// penceresi tekrar tetikleniyor) -- toggleScreenShare'in kendisi idempotent DEĞİL, "şu an
// ne varsa onun tersini yap" mantığında, bu yüzden çift tetiklenme doğrudan görünür hale
// geliyor.
//
// Kesin kök neden (globalShortcut'ın Windows'ta bazı kombinasyonlarda callback'i iki kez
// çağırdığı bilinen Electron davranışı mı, yoksa inputHook.js'teki mousedown press
// handler'ının -- klavyenin aksine -- "held" korumasız olması/fare tuşu "chatter"ı mı)
// canlı ortamda kesinleştirilemedi. Ama HANGİ kaynaktan gelirse gelsin aynı belirtiyi
// verdiği için, TEK bir merkezi noktada (uygulanan HER kısayolun tetiklenme anında) kısa
// bir zaman penceresi içindeki tekrar çağrıları yutan genel bir debounce -- ayrı ayrı her
// eyleme özel yama eklemek yerine -- en sağlam ve bakımı en kolay çözüm.
