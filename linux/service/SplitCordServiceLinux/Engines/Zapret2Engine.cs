...
/// <summary>
/// Otomatik moddaki ikinci motor (Zapret tükenirse, bkz. PORTING_PLAN.md D-2): bol-van/zapret2'nin
/// NATIVE Linux binary'si (nfqws2 — Windows'taki winws2.exe'nin Cygwin'e bağımlı olmayan asıl
/// hâli). ZapretEngine'in aksine sabit/elle yazılmış bir aday listesi kullanmıyor — Zapret2'nin
/// kendi resmi keşif aracı olan blockcheck2'yi (bkz. linux/resources/bin/zapret2/blockcheck2/
/// blockcheck2.sh) BATCH=1 ile programatik olarak çalıştırıp discord.com için gerçekten çalışan
/// bir nfqws2 stratejisi arıyor. blockcheck2 yalnızca TCP/TLS/HTTP3 test ediyor (UDP/ses'e hiç
/// dokunmuyor) — bu yüzden bulunan strateji ile nfqws2 başlatıldıktan sonra ayrıca bir STUN
/// Binding Request/Response (RFC 5389) ile ham UDP'nin bu strateji altında gerçekten dışarı
/// çıkıp çıkamadığı da test ediliyor (bkz. VerifyVoiceAsync). blockcheck2 hiç strateji
/// bulamazsa (ya da bulduğu hiçbiri gerçek discord.com bağlantısında doğrulanamazsa)
/// AllCandidatesFailedException fırlatılır — DpiEngineManager bunu yakalayıp ByeDPI'ye
/// eskalasyon yapar.
///
/// Windows karşılığının (service/SplitCordService/Engines/Zapret2Engine.cs) portu — ana fark:
/// Windows'taki Cygwin makinesi (bundled bash.exe + cygwin1.dll bağımlılığı, /cygdrive/
/// yol çevirisi, eksik dizin/DLL-bulma iş yükleri) TAMAMEN DÜŞTÜ — native Linux'ta bash zaten
/// sistemde var, nfqws2 native bir ELF binary. Paket YAKALAMA da ZapretEngine'deki gibi
/// iptables/NFQUEUE'ya taşındı (bkz. SpawnAsync). DOĞRULANMADI (bkz. PORTING_PLAN.md §2
/// madde 5, R-4.1/R-4.2/R-4.3 ve aşağıdaki ek notlar): hiç gerçek bir Linux'ta çalıştırılmadı.
/// EN BÜYÜK AÇIK VARSAYIM: blockcheck2.sh'nin native Linux sürümünün KENDİ iç test döngüsü
/// için KENDİ iptables/NFQUEUE kuralını yönettiği varsayılıyor (Windows'ta WinDivert filtresini
/// winws2.exe'nin kendi CLI argümanlarına gömüyordu, bu artık mümkün değil) — bu doğrulanana
/// kadar RunBlockcheck2Async'in kendisi HİÇBİR iptables kuralı KURMUYOR, yalnızca script'i
/// çalıştırıp çıktısını okuyor. Script'in gerçekten kendi kuralını kurmadığı ortaya çıkarsa bu
/// metoda da ZapretEngine.SpawnAsync'teki gibi bir NFQUEUE kurulumu eklenmesi gerekecek.
/// </summary>
public sealed class Zapret2Engine : IDpiEngine, IDnsTierAware
{
    ...
    // çok daha erken, iki ayrı satır hâlinde basıyor (Windows tarafında canlı testte doğrulanan
    // format, "winws2" yerine "nfqws2" ile — bkz. DOĞRULANMADI notu):
    //   "- curl_test_http ipv4 discord.com : nfqws2 <strateji argümanları>"
    //   "!!!!! AVAILABLE !!!!!"  (ya da "curl: (NN) ..." + "UNAVAILABLE code=NN")
    // Windows tarafında canlı testte doğrulandı: bu ilk AVAILABLE'a kadar dakikalarca sürebilen
    // aynı grup içinde onlarca varyasyon daha deneniyor — yalnızca özet satırını beklemek,
    // çoktan AVAILABLE çıkmış bir adayı kaçırıp taramayı gereksiz uzatıyordu. Bu yüzden aday
    // satırını (aşağıdaki regex) bir sonraki AVAILABLE ile eşleştirip AYNI erken-durdurma
    // mekanizmasını (earlyStopCts) çok daha erken tetikliyoruz.
    //
    // DOĞRULANMADI: Windows portu blockcheck2 çıktısında daemon adı olarak "winws2" arıyordu
    // (winws2.exe'nin kendi adı) — native Linux binary'sinin adı "nfqws2" olduğu için burada
    // "nfqws2" aranıyor, ama blockcheck2.sh'nin native Linux çıktısının GERÇEKTEN bu ismi
    // kullandığı doğrulanmadı (upstream script farklı bir isim/yol da basıyor olabilir).
    private static readonly Regex CandidateLineRegex = new(
        @"^-\s+curl_test\S*\s+ipv\d+\s+\S+\s*:\s*nfqws2\s+(?<strategy>.+)$",
        RegexOptions.Compiled);
    ...
    // da aynen uygulanıyor): Zapret ile PAYLAŞILAN, kullanıcı tarafından düzenlenebilen
    // hostlist-exclude dosyası (bkz. HostlistManager.cs).
    ...
    /// <summary>blockcheck2.sh'yi (bkz. linux/resources/bin/zapret2/blockcheck2/) SİSTEMİN
    /// KENDİ bash'iyle BATCH=1 (etkileşimli soruları atlar) ve DOMAINS=discord.com ile
    /// çalıştırır, stdout'unu satır satır okuyup "!!!!! ... working strategy found ...
    /// !!!!!" desenindeki, daemon'u nfqws2 olan satırları ayrıştırır.
    ///
    /// Windows karşılığından FARK: bundled Cygwin bash.exe yerine "bash" (PATH üzerinden
    /// çözümlenir — Linux'ta bash neredeyse evrensel olarak /bin/bash'te, ayrıca çoğu
    /// dağıtımda /usr/bin/bash'e sembolik bağlantı). Windows'taki cygwinDir PATH-injection'ı,
    ...
    private async Task<List<string>> RunBlockcheck2Async(CancellationToken ct, bool forceDoh, HashSet<string> excludeCandidates, TimeSpan remainingBudget)
    {
        ...
        // Windows'taki AYNI kararlılık önlemi (WinDivert'in "tek işleyici" kısıtlaması NFQUEUE'da
        // "aynı kuyruk numarasına tek bağlanan" kısıtlamasına karşılık geliyor): blockcheck2.sh'nin
        // KENDİ iç test döngüsünden kalma bir nfqws2 hâlâ ayaktaysa, YENİ bir tarama başlamadan
        // önce temizliyoruz. Yalnızca GERÇEKTEN bir şey öldürüldüyse bekliyoruz -- bkz.
        // KillStrayNfqws2Processes'in dönüş değeri üstündeki not.
        if (KillStrayNfqws2Processes()) await Task.Delay(300, ct);
        ...
        // NOT: -i (etkileşimli) veya --login KULLANMIYORUZ — Windows tarafında canlı testte
        // doğrulanan AYNI gerekçe (yönlendirilmiş stdio ile gerçek bir TTY olmadan sorunlu
        // olabiliyorlar) burada da geçerli sayılıyor; düz (non-interactive, non-login) bash
        ...
        process.OutputDataReceived += (_, e) =>
        {
            ...
            if (match.Success && string.Equals(match.Groups["daemon"].Value, "nfqws2", StringComparison.OrdinalIgnoreCase))
            {
        ...
        finally
        {
            ...
            // bkz. KillStrayNfqws2Processes üstündeki not: blockcheck2.sh'nin KENDİ iç test
            // döngüsünden kalma nfqws2 örnekleri olabilir — bunlar temizlenmeden bir sonraki
            // SpawnAsync kendi NFQUEUE kuyruğuna bağlanamayabilir.
            KillStrayNfqws2Processes();
        }

        _logger.LogInformation("blockcheck2 tamamlandı, {Count} nfqws2 adayı bulundu", candidates.Count);
    ...
    public async Task StopAsync(CancellationToken ct)
    {
        if (_process is { HasExited: false } process)
        {
            try
            {
                ...
                // Bir sonraki motor/deneme başlamadan önce bu sürecin (ve tuttuğu NFQUEUE
                // kuyruğunun) GERÇEKTEN kapandığından emin olmak için kısa bir süre bekliyoruz
                // (Kill() OS seviyesinde asenkron).
        ...
        // Windows tarafındaki AYNI kararlılık önlemi: blockcheck2.sh'nin KENDİ iç test
        // döngüsünden kalma, _process/_blockcheckProcess alanlarımızca hiç TAKİP EDİLMEYEN
        // nfqws2 örnekleri olabilir. Motor durdurulduğunda sistem genelinde GERÇEKTEN sıfır
        // nfqws2 kalmasını garanti ediyoruz.
        KillStrayNfqws2Processes();
    }
    ...
    /// TEK bir süreç bağlanabilir. blockcheck2.sh KENDİ iç test döngüsünde kısa ömürlü nfqws2
    /// süreçleri başlatıp durduruyor — bunlar _blockcheckProcess/_process alanlarımızca TAKİP
    /// EDİLMİYOR. Adı geçen HER nfqws2 sürecini sistem genelinde zorla sonlandırıp bir sonraki
    /// spawn'ın GERÇEKTEN boş bir kuyrukla başlamasını garanti ediyoruz.
    ///
    /// KULLANICI TALEBİ (Windows karşılığıyla AYNI): gerçekten öldürülen bir şey olup olmadığını
    /// bool olarak döndürüyor — çağıranlar bunu, ardından gelen 300ms'lik "kuyruk serbest
    /// kalsın" beklemesini YALNIZCA gerçekten bir süreç öldürüldüyse yapmak için kullanıyor.
    /// Asıl/yaygın durum (artık nfqws2 YOK) bu sayede beklemeden atlanıyor.</summary>
    private bool KillStrayNfqws2Processes()
    {
        Process[] stray;
        try { stray = Process.GetProcessesByName("nfqws2"); }
        catch { return false; }
    ...
    // Zapret2'nin doğrudan (blockcheck2 dışında, kazanan stratejiyle) spawn ettiği nfqws2 için
    // NFQUEUE kuyruk numarası — ZapretEngine'in 100/101'inden FARKLI (bkz. ZapretEngine.cs'teki
    // NfQueueNum/UdpCompanionNfQueueNum) ki iki motor arka arkaya denenirken (escalation) kalıntı
    // bir kural çakışmasın.
    private const int NfQueueNum = 102;

    /// <summary>nfqws2'yi verilen argümanlarla, kendi NFQUEUE kuyruğu kurulduktan sonra başlatır
    /// (ZapretEngine.SpawnAsync ile aynı desen — nfqws2 tabanlı motorların CLI argüman yüzeyi
    /// nfqws ile uyumlu).
    ///
    /// Windows karşılığından FARK: cygwin1.dll bulma amaçlı PATH-injection (winws2.exe bir
    /// Cygwin ikiliydi) TAMAMEN KALKTI — nfqws2 native bir ELF binary, hiçbir DLL/PATH
    /// bağımlılığı yok. --lua-init bayrakları KORUNDU (bkz. aşağıdaki not) çünkü bu,
    /// blockcheck2.sh'nin --lua-desync tekniklerini uygulama şekli — platformdan bağımsız bir
    ...
    private async Task SpawnAsync(string args, CancellationToken ct)
    {
        ...
        // nfqws2'yi (blockcheck2'nin kendi iç testinden ya da önceki bir adaydan kalma)
        // temizliyoruz — bkz. üstteki KillStrayNfqws2Processes notu. Yalnızca GERÇEKTEN bir
        // şey öldürüldüyse bekliyoruz (asıl/yaygın durumda öldürülecek bir artık YOK).
        if (KillStrayNfqws2Processes()) await Task.Delay(300, ct);

        await AddNfQueueRulesAsync();
        ...
        // blockcheck2.sh nfqws2'yi HER ZAMAN "--lua-init=@zapret-lib.lua --lua-init=@zapret-
        // antidpi.lua" ile başlatıyor (Windows tarafında script kaynağından doğrulandı,
        // blockcheck2.sh:951/954/962 — pktws_start) — bu iki Lua modülü, "--lua-desync=X"
        // adıyla verilen tekniklerin GERÇEK implementasyonlarını içeriyor. blockcheck2'nin
        // bulduğu bir aday bu bayrağı gerektiriyorsa, biz KENDİ SpawnAsync'imizde de (blockcheck2
        // dışında, doğrudan) başlatırken AYNI iki bayrağı eklememiz gerekiyor, yoksa "--lua-
        // desync=X" implementasyonu bulunamaz. DOĞRULANMADI: bu iki .lua dosyasının native Linux
        // paketindeki GERÇEK yolu (blockcheck2Dir/lua/...) — Windows bundle'ındakiyle aynı göreli
        // konumda olduğu VARSAYILIYOR.
        ...
        _logger.LogInformation("Zapret2 nfqws2 tam komut satırı: {Exe} {Args}", exePath, string.Join(' ', psi.ArgumentList));
        ...
        process.Exited += (sender, _) =>
        {
            try
            {
                ...
                _logger.LogWarning("Zapret2 (nfqws2) beklenmedik şekilde durdu (ExitCode={ExitCode})", exitCode);
                _logs.Add($"[tanı] nfqws2 durdu, ExitCode={exitCode}");
            }
    ...
    /// <summary>DNS çözümlemesi SelfTestResolver ile yapılıyor — ByeDPI'nin (ciadpi) zaten
    /// konuştuğu AYNI yerel EncryptedDnsForwarder'a (bkz. Dns/SelfTestResolver.cs) düz DNS-wire
    ...
    /// <summary>ZapretEngine.cs'teki AYNI desen (bkz. oradaki DOĞRULANMADI notu, PORTING_PLAN.md
    /// R-4.2) — TCP 80/443 ve UDP 443,50000-65535'i SpawnAsync'in NfQueueNum'una (102)
    /// yönlendiren iptables kuralları. Yalnızca bizim KENDİ doğrudan nfqws2 spawn'ımız için —
    /// RunBlockcheck2Async'in kendi iç test döngüsü için (bkz. o metodun DOĞRULANMADI notu)
    /// AYRI/kendi kuralını yönettiği varsayılıyor, bu yüzden burada çağrılmıyor.</summary>
    private static async Task AddNfQueueRulesAsync()
    {
        await RunIptablesAsync("-I", "OUTPUT", "-p", "tcp", "-m", "multiport", "--dports", "80,443",
            "-j", "NFQUEUE", "--queue-num", NfQueueNum.ToString(), "--queue-bypass");
        await RunIptablesAsync("-I", "OUTPUT", "-p", "udp", "-m", "multiport", "--dports", "443,50000:65535",
            "-j", "NFQUEUE", "--queue-num", NfQueueNum.ToString(), "--queue-bypass");
    }

    private static async Task RemoveNfQueueRulesAsync()
    {
    ...
    private static async Task RunIptablesAsync(params string[] args)
    {
        ...
        try
        {
            ...
            if (process is null) return;
            await process.WaitForExitAsync().WaitAsync(TimeSpan.FromSeconds(5));
            // bkz. ZapretEngine.cs'teki AYNI not: -D başarısızlığı (kural zaten yoksa) ya da -I
            // başarısızlığı (yetki yoksa) burada sessizce yutuluyor -- nfqws2'nin kendi başlatma
            // hatası zaten ayrı bildirilecek.
        }
        catch
        {
