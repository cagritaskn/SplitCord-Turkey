...
// GERÇEK BUG (kullanıcı raporu, canlı DOM'da doğrulandı): Kamera/Ekran Paylaşımı aç/kapat
// kısayolları, YALNIZCA sesli bağlantıda bulunulan sunucu/kanal o an EKRANDA SEÇİLİYKEN
// çalışıyordu. Kök neden: sol alttaki HER ZAMAN görünen "Ses Bağlantısı Kuruldu" kompakt
// panelindeki Kamera/Ekran Paylaşımı butonlarının `aria-label` özniteliği YOK -- bunun
// yerine `aria-describedby` ile ayrı bir tooltip elemanına işaret ediyorlar (o eleman
// hover ETMEDEN de DOM'da mevcut ve okunabilir, yalnızca görünürlüğü hover'a bağlı).
// Kanalın TAM (genişletilmiş) araç çubuğu -- YALNIZCA o kanal ekrandayken var olan --
// AYNI düğmelerin `aria-label` KULLANAN bir kopyasını render ediyor; eski
// `document.querySelectorAll('[aria-label]')` sorgumuz kompakt paneldeki (her zaman
// mevcut) düğmeleri bu yüzden hiç bulamıyordu. getAccessibleLabel() ikisini de kapsıyor:
// önce doğrudan aria-label'a bakıyor, yoksa aria-describedby'nin işaret ettiği elemanın
// metnine düşüyor -- hangi görünüm render edilmiş olursa olsun aynı düğmeyi buluyor.
...
// GERÇEK BUG (kullanıcı raporu): ekran paylaşımı seçici penceresi AÇIKKEN (henüz bir
