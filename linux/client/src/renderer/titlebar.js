...
// CANLI TESTTE BULUNAN GERÇEK BUG (2026-09-07): hem did-finish-load'daki "gerçekten Discord'a
// mı ulaşıldı" kontrolü hem de did-fail-load'ın "sayfa zaten kendiliğinden toparlandı mı"
// kontrolü webview.getURL()'e güveniyordu -- ama bir yükleme ERR_CONNECTION_RESET gibi bir
// hatayla Chromium'un kendi dahili hata sayfasına (chrome-error://chromewebdata/) düşünce,
// getURL() GERÇEKTE gösterilen chrome-error:// adresini DEĞİL, DENENEN hedefi (ör.
// "https://discord.com/login") döndürmeye devam ediyordu -- discordWebviewPreload.js'in
// KENDİSİ (sayfanın İÇİNDEN location.href okuyarak, bkz. discord-preload-diag log'u) doğru
// adresi (chrome-error://...) görüyordu, ama titlebar.js'teki bu iki kontrol yanlış pozitif
...
// sayfanın KENDİ location.href'ini (executeJavaScript ile, discord-preload-diag'in yaptığının
// AYNISI) okuyoruz -- hata sayfalarında bile script çalıştırma çalışıyor (log'da doğrulandı).
...
// ya da (Windows'ta) Kaspersky/ESET tespit edildiği için Zapret hiç denenemediğinde — kullanıcıya
...
btnStatusRetryAuto?.addEventListener('click', async () => {
  const choice = await window.showConfirmModal({
    ...
    message: 'Zapret, Zapret2 ve ByeDPI ayarları en baştan sırayla denenecek.',
...
// de karşılık gelen getSystemControlsStatus yok, antivirusInfo.js dosyası da bu yüzden
// tamamen kaldırıldı). Çağıran yer (refreshConnection) değiştirilmeden bırakıldı, bu
// fonksiyon bilerek no-op.
...
async function refreshConnection() {
  ...
  try {
    ...
    // Zapret2'nin kendi taramasına otomatik geçilirken) o anki adayın "running=false"
    ...
    // servis tarafında bu tespit yok, bkz. PORTING_PLAN.md D-9 — kontrol zararsız kalsın diye
    // koddan çıkarılmadı.)
    ...
    if (active?.running) {
      ...
      // (ör. Zapret/Zapret2'nin "fake packet" tekniğinin birkaç denemede oturması gereken
...
// Canlı testte (Windows'ta) gözlemlendi: fake-packet tekniği bazen 4-5 deneme sonra
...
webview?.addEventListener('did-fail-load', async (event) => {
  ...
  // ERR_QUIC_PROTOCOL_ERROR (ve QUIC'in diğer hata türleri): Windows'ta hiçbir DPI aşım
  // motoru QUIC/UDP trafiğine dokunmuyordu (WinDivert filtreleri yalnızca --wf-tcp-out ile
  // TCP'yi hedefliyordu). DOĞRULANMADI (bkz. PORTING_PLAN.md §2 madde 5): Linux'ta
  // ZapretEngine'in NFQUEUE kuralı UDP 443'ü de (QUIC'in kullandığı port) kapsıyor, ama
  // mevcut aday stratejilerinin hiçbiri QUIC'e özel bir teknik içermiyor (hepsi TCP/TLS
  // odaklı) — bu yüzden pratikte hâlâ hiçbir strateji değişikliğinin bunu düzeltmesi
  // beklenmiyor, "QUIC'i kapat" seçeneği burada bilinçli olarak korundu.
  // Bkz. btnStatusDisableQuic click handler'ı / ipc.js app:set-quic-disabled.
  ...
  try {
    ...
    if (engineFailCount > ENGINE_MAX_AUTO_RETRIES) {
      ...
      // (bu ERR_CONNECTION_RESET'in ait olduğu reload'un kendisi milisaniyeler içinde toparlanıp
      // did-finish-load'u ZATEN tetiklemiş olabilir — Zapret/Zapret2'nin paket-seviyesi
      // tekniklerinde bu son derece sık) başarıyla yüklenmiş olabiliyordu -- aşağıdaki
      // refreshConnection() bunu HİÇ KONTROL ETMEDEN koşulsuz webview.reload() çağırıyordu,
      // bu da ZATEN İYİ ÇALIŞAN sayfayı gereksiz yere yeniden yükleyip AYNI geçici hatayı
      // (ve dolayısıyla AYNI 2sn'lik yeniden deneme döngüsünü) yeniden tetikliyordu --
      // "Discord yükleniyor…" ekranının sürekli flaşlanmasının kök nedeni buydu. Düzeltme:
      // yeniden yüklemeden önce sayfanın zaten gerçekten yüklü/başarılı olup olmadığını
      // kontrol ediyoruz; öyleyse gereksiz reload'u ATLIYORUZ.
...
// hemen görmeyince sabırsızlanıp AYNI saniye içinde tekrar tıklıyor, bu da güncelleme kurulumunun
