...
// Zapret2Engine.cs'teki SavedArgsRetryAttempts notundan bilindiği gibi winws2.exe'nin
// WinDivert'e oturması diğer motorlara göre daha kırılgan olabiliyor: strateji GERÇEKTEN
// çalışıyor olsa bile Discord'un kendi sayfası (statik "BİLİYOR MUYDUN?" kabuğu — bkz.
// discordWebviewPreload.js setupBanCurrentArgsButton'daki AYNI discordstatus.com/twitter.com/
// x.com footer bağlantısı tespiti) uzun süre takılı kalabiliyor, GERÇEK bir did-fail-load
// tetiklemeden (üst seviye sayfa zaten başarıyla yüklendi — takılan Discord'un KENDİ iç
// JS/WebSocket mantığı) — bu yüzden aşağıdaki did-fail-load tabanlı, TÜM motorlar için ortak
// yeniden deneme mekanizması bu durumu hiç YAKALAYAMIYOR. Bu ayrı, yalnızca Zapret2'ye özel
// bekçi 30 saniyede bir yoklayıp (hem "hâlâ aynı takılı kabuk" hem "gerçekten bir hata
// sayfasına düştü" durumlarını TEK kontrolde birleştirerek) gerekiyorsa sayfayı otomatik
// yeniden yüklüyor — en fazla 5 kez; 5. denemeden sonra otomatik yenilenmiyor (kullanıcı
// hâlâ elle "Argüman Setini Yasaklamayı Deneyin" butonuna basabilir).
