...
/// <summary>
/// Varsayılan/ana DPI aşım motoru. ciadpi'yi (hufrea/byedpi) yerel bir SOCKS5 proxy
...
///
/// Windows karşılığının (service/SplitCordService/Engines/ByeDpiEngine.cs) portu — aday
/// argüman listeleri (hufrea/byedpi CLI sözdizimi, platformdan bağımsız) BİREBİR aynı,
/// yalnızca ikili adı "ciadpi.exe" -> "ciadpi" oldu. Süreç başlatma/SOCKS5 bağlantı testi
/// tamamen taşınabilir .NET API'leri (Process, TcpClient, HttpClientHandler+WebProxy)
/// kullanıyor. DOĞRULANMADI: linux/vendor/byedpi-src'den gerçek bir Linux ciadpi binary'si
/// henüz derlenip bu motorla test edilmedi (bkz. PORTING_PLAN.md Faz 3).
/// </summary>
public sealed class ByeDpiEngine : IDpiEngine, IDnsTierAware
{
    ...
    private static readonly string[] ShortCandidateStrategies =
    {
        ...
        // fuzzer çıktısındaki --drop-sack, bu projede pinlenen ciadpi sürümünde
    ...
    public async Task StartAsync(CancellationToken ct)
    {
        ...
        // listesinin TAMAMI o tier aktifken denenir. ciadpi kendi DNS sorgusunu HER ZAMAN
    ...
    /// <summary>ciadpi'yi verilen argümanlarla başlatır. _process alanı yalnızca
    ...
    {
        var exePath = BinaryLocator.Resolve("byedpi", "ciadpi");
        ...
        process.Exited += (_, _) => { try { _logger.LogWarning("ByeDPI (ciadpi) beklenmedik şekilde durdu"); } catch { /* bkz. yukarıdaki not */ } };
