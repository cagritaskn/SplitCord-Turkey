...
/// <summary>Servisin (ve LocalApiEndpoints üzerinden istemcinin) TÜM tanılama günlüğünü tek
/// bir dosyada topluyor: %ProgramData%\SplitCord\diagnostic.log — SettingsStore'un
/// service-settings.json için kullandığı AYNI dizin (servis SYSTEM oturumunda çalıştığı için
/// kullanıcı profiline değil ortak makine dizinine yazılabiliyor). Dosya 50MB'a ulaştığında
/// TAMAMEN temizlenip bir "temizlendi" satırıyla sıfırdan başlıyor (basit tek-dosya sınırlama —
/// ayrı bir .old dosyası tutulmuyor, "max 50MB'lık BİR log" isteğiyle birebir örtüşsün diye).</summary>
public sealed class DiagnosticLogWriter
{
    ...
    {
        var dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "SplitCord");
        Directory.CreateDirectory(dir);
        _path = Path.Combine(dir, "diagnostic.log");
    }
...
/// <summary>Mevcut ILogger<T> altyapısına (tüm motorlar, DpiEngineManager, DNS forwarder vs.
/// zaten bunu kullanıyor) tek bir provider eklenerek servisin ürettiği HER log satırının
/// ayrıca DiagnosticLogWriter'a da yazılmasını sağlıyor — motor/altyapı dosyalarının
/// hiçbirine dokunmaya gerek kalmıyor.</summary>
