...
// saygı göstermiyordu. nativeTheme.themeSource, Windows'ta TÜM yeni pencerelerin native
// başlık çubuğu/çerçeve rengini doğrudan etkiliyor (DWM API); Linux'ta ETKİ MASAÜSTÜ
// ORTAMINA/pencere yöneticisine bağlı -- GTK tabanlı ortamlarda prefers-color-scheme
// üzerinden dolaylı olarak etkili olabilir ama Windows'taki gibi garanti bir native
// başlık çubuğu rengi DEĞİŞTİRMEZ. Yine de zararsız/standart bir Electron API'si olduğu
// için tutarlılık amacıyla burada da uygulanıyor. dynamicColor.js'in canlı örneklediği
// palete göre (isDark) senkronize ediliyor. (Ana pencerenin kendi özel/çerçevesiz başlık
// çubuğu zaten theme.css üzerinden ayrı yönetiliyor, bundan etkilenmiyor.)
...
function createMainWindow() {
  ...
  mainWindow.webContents.on('did-attach-webview', (_event, webContents) => {
    ...
    webContents.setWindowOpenHandler(({ url, features }) => {
      ...
      // Aktivite'yi (Wordle) "Pencere Modu"na alınca discord.com kökenli bir URL sistem
      // tarayıcısında açılıyordu): "harici link" kontrolü discord.com'un KENDİ
      // pencerelerini (aktivite/yayın "Pencere Modu" popout'ları) de "harici" sayıyordu,
      // çünkü tek kontrol http(s) olup olmadığıydı, kökene hiç bakmıyordu. discord.com (ve
      // alt kökenleri) kökenli bir pencere GERÇEKTEN harici DEĞİL -- bu yüzden
      // permissions.js'teki AYNI izin verilen köken listesiyle (isAllowedPopoutOrigin --
      // discordsays.com/Aktivite iframe'lerini de kapsayan AYRI/daha geniş bir liste,
      // bkz. o dosyadaki not) süzülüyor; yalnızca GERÇEKTEN başka bir siteye (ör. bir
      // mesajdaki linke tıklanması) giden pencereler "harici" sayılıyor. KULLANICI TALEBİ
      // ÜZERİNE: discord.com kökenli pencereler "Bağlantıları varsayılan sistem
      // tarayıcısında aç" ayarından TAMAMEN BAĞIMSIZ olarak HER ZAMAN uygulama içinde
      // açılır -- o ayar yalnızca GERÇEKTEN harici sitelere uygulanıyor.
    ...
    webContents.on('did-create-window', (childWindow, details) => {
      ...
      // GERÇEK BUG (kullanıcı raporu, Windows istemcisinde canlı testte doğrulandı):
      // Aktivite (Wordle) "Etkinlikten Ayrıl" ile bitirildiğinde, popout penceresi KENDİ
      // KENDİNE KAPANMIYORDU -- Discord'un popout sayfası (discord.com/popout) aktivite
